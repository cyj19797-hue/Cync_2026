...
//
//  Figma's back chevron + centered title (`240:3487`/`361:2593`) is standard
//  NavigationStack chrome — this view is pushed via `NavigationLink` from
//  CalendarView (which already owns the NavigationStack), so the back
//  button and title come from `.navigationTitle` for free; no custom back
//  button view is built here.
//
